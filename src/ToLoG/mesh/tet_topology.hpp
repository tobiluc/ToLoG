...
private:
    using bits = uint8_t;
    template<typename T>
    constexpr static bits b(const T _t) {
...
public:
    ...
    constexpr static V i2v(int _i) {
        return (_i>=0&&_i<4)? v(static_cast<uint8_t>(_i)) : V::O;
    }
    constexpr static HF i2hf(int _i) {
        return (_i>=0&&_i<4)? opp_hf(i2v(_i)) : HF::O;
    }
    constexpr static bool is_vertex(VAR _var) {
        return (b(_var)&0b11000000)==0b01000000;
    }
    constexpr static bool is_halfedge(VAR _var) {
        return (b(_var)&0b11000000)==0b10000000;
    }
    constexpr static bool is_halfface(VAR _var) {
        return (b(_var)&0b11000000)==0b11000000 && !is_tet(_var);
    }
    constexpr static bool is_tet(VAR _var) {
        return (b(_var)&0b11111100)==0b11111100;
    }
    constexpr static bool is_valid(VAR _var) {
        return (b(_var)&0b11000000)!=0b00000000;
    }
    constexpr static VAR var(V _v) {
        return static_cast<VAR>(_v);
    }
    constexpr static VAR var(HE _he) {
        return static_cast<VAR>(_he);
    }
    constexpr static VAR var(HF _hf) {
        return static_cast<VAR>(_hf);
    }
    constexpr static V v(const VAR _var) {
        return is_vertex(_var)? static_cast<V>(_var) : V::O;
    }
    constexpr static HE he(const VAR _var) {
        return is_halfedge(_var)? static_cast<HE>(_var) : HE::O;
    }
    constexpr static HF hf(const VAR _var) {
        return is_halfface(_var)? static_cast<HF>(_var) : HF::O;
    }
    constexpr static bool is_same_halfface(HF _hf1, HF _hf2) {
        return opp_v(_hf1)==opp_v(_hf2);
    }
    constexpr static bool is_same_edge(HE _he1, HE _he2) {
        return _he1==_he2||_he1==opp(_he2);
    }
    constexpr static HE he(V _v0, V _v1) {
        return he((0b10<<6)|(i(_v1)<<2)|i(_v0));
    }
    constexpr static HF hf(V _v0, V _v1, V _v2) {
        return hf((0b11<<6)|((b(_v2)&3)<<4)|((b(_v1)&3)<<2)|(b(_v0)&3));
    }
    constexpr static V v(HE _he, uint8_t _idx) {
        return v((0b01<<6)|((b(_he)&(3<<(_idx<<1)))>>(_idx<<1)));
    }
    constexpr static V v0(HE _he) {
        return v(_he,0);
    }
    constexpr static V v1(HE _he) {
        return v(_he,1);
    }
    constexpr static V v0(HF _hf) {
        return v(_hf,0);
    }
    constexpr static V v1(HF _hf) {
        return v(_hf,1);
    }
    constexpr static V v2(HF _hf) {
        return v(_hf,2);
    }
    constexpr static V v(HF _hf, uint8_t _idx) {
        return v((0b01<<6)|((b(_hf)&(3<<(_idx<<1)))>>(_idx<<1)));
    }
    constexpr static HE opp(HE _he) {
        return he(v(_he,1), v(_he,0));
    }
    constexpr static V opp_v(HF _hf) {
        return fourth_vertex(v(_hf,0), v(_hf,1), v(_hf,2));
    }
    constexpr static HF opp_hf(V _v) {
        return is_valid(var(_v))? v_opp_hf_[i(_v)] : HF::O;
    }
    constexpr static HE he(HF _hf, uint8_t _idx) {
        return he(v(_hf,_idx), v(_hf,(_idx+1)%3));
    }
    constexpr static HE he0(HF _hf) {
        return he(_hf, 0);
    }
    constexpr static HE he1(HF _hf) {
        return he(_hf, 1);
    }
    constexpr static HE he2(HF _hf) {
        return he(_hf, 2);
    }
    constexpr static bool contains(HE _he, V _v) {
        return v(_he,0) == _v || v(_he,1) == _v;
    }
    constexpr static bool contains(HF _hf, V _v) {
        return v(_hf,0) == _v || v(_hf,1) == _v || v(_hf,2) == _v;
    }
    constexpr static const std::array<HF,3> incident_halffaces(V _v) {
        return v_inc_hf_[i(_v)];
    }
    constexpr static const std::array<HF,2> incident_halffaces(V _v1, V _v2) {
        assert(_v1!=_v2);
    ...
    constexpr static const std::array<HE,3> incident_halfedges(V v) {
        return {he(v,add<1>(v)),he(v,add<2>(v)),he(v,add<3>(v))};
    }
    constexpr static const HE he(HF _hf1, HF _hf2) {
        for (int i = 0; i < 3; ++i) {
    ...
    // constexpr static const HF incident_halfface(HE _he) {
    //     // const V a = v0(_he);
    //     // const V b = v1(_he);
    ...
    // }
    // constexpr static const HE next_halfedge(HF _hf, HE _he) {
    //     return _he;
    // }

private:
    constexpr static V v(const bits _b) {return static_cast<V>(0b01000000|(_b&3));}
    constexpr static HE he(const bits _b) {return static_cast<HE>(_b);}
    constexpr static HF hf(const bits _b) {return static_cast<HF>(_b);}
    ...
    template<uint8_t a>
    constexpr static V add(V _v) {
        return v(0b01000000|((b(_v)+a)&3));
    }
    constexpr static V fourth_vertex(V _v0, V _v1, V _v2) {
...
public:
    constexpr Vertex vertex(TT::V _v) const {
        return TT::is_valid(TT::var(_v))? v_[TT::i(_v)] : Vertex();
    }
    constexpr HalfEdge halfedge(TT::HE _he) const {
        return TT::is_valid(TT::var(_he))? he_[TT::i(_he)] : HalfEdge();
    }
    constexpr HalfFace halfface(TT::HF _hf) const {
        return TT::is_valid(TT::var(_hf))? hf_[TT::i(_hf)] : HalfFace();
    }
    constexpr Vertex operator[](TT::V _v) const {
        return vertex(_v);
    }
    constexpr HalfEdge operator[](TT::HE _he) const {
        return halfedge(_he);
    }
    constexpr HalfFace operator[](TT::HF _hf) const {
        return halfface(_hf);
    }
    constexpr std::optional<std::variant<Vertex,HalfEdge,HalfFace,Cell>> operator[](TT::VAR _var) const {
    ...
    constexpr const std::array<Vertex,4>& vertices() const {
        return v_;
    }
    constexpr const std::array<HalfEdge,12>& halfedges() const {
        return he_;
    }
    constexpr const std::array<HalfFace,4>& halffaces() const {
        return hf_;
    }
    constexpr const Cell& cell() const {
        return c_;
    }
    constexpr std::array<Vertex,2> vertices(TT::HE _he) const {
        return {vertex(TT::v0(_he)),vertex(TT::v1(_he))};
    }
    constexpr std::array<Vertex,3> vertices(TT::HF _hf) const {
        return {vertex(TT::v0(_hf)),vertex(TT::v1(_hf)),vertex(TT::v2(_hf))};
    }
    constexpr std::array<HalfEdge,3> halfedges(TT::HF _hf) const {
        return {halfedge(TT::he0(_hf)),halfedge(TT::he1(_hf)),halfedge(TT::he2(_hf))};
    }
    constexpr TT::V v(Vertex _vh) const {
        for (int idx = 0; idx < 4; ++idx) {if (v_[idx] == _vh) {return TT::i2v(idx);}}
        return TT::V::O;
    }
    constexpr TT::HE he(HalfEdge _heh) const {
        for (int idx = 0; idx < 12; ++idx) {if (he_[idx] == _heh) {return TT::i2he(idx);}}
        return TT::HE::O;
    }
    constexpr TT::HF hf(HalfFace _hfh) const {
