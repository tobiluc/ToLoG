...
TEST(IntersectsTest, EllipsoidTest)
{
    ...
    AABB bbox = aabb(e);
    EXPECT_NEAR(bbox.min()[0], 0, eps);
    EXPECT_NEAR(bbox.max()[0], 2, eps);
    EXPECT_NEAR(bbox.min()[1], -2, eps);
    EXPECT_NEAR(bbox.max()[1], 2, eps);
    EXPECT_NEAR(bbox.min()[2], -3, eps);
    EXPECT_NEAR(bbox.max()[2], 3, eps);
}
